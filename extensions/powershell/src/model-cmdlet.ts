...
export class ModelCmdlet extends Class {
  ...
  constructor(namespace: Namespace, schema: Schema, state: State, objectInitializer?: Partial<ModelCmdlet>) {
    const name = `New${state.model.details.csharp.nounPrefix}${schema.details.csharp.name}Object_${schema.details.csharp.apiname}`;
...
function addClassAttributes($class: WithState, schema: Schema, name: string) {
  ...
  $class.add(new Attribute(CmdletAttribute, { parameters: [`System.Management.Automation.VerbsCommon.New`, new StringExpression(`${$class.state.model.details.csharp.nounPrefix}${schema.details.csharp.name || ''}Object_${schema.details.csharp.apiname}`)] }));
